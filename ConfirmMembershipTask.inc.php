...
class ConfirmMembershipTask extends ScheduledTask {
    ...
    private function userHasSubmission($userId, $jobsAutoDeleteAge) {
        ...
        $checkUserSubmissions = $stageAssignmentDao->retrieve("select *  from stage_assignments join submissions on stage_assignments.submission_id =submissions.submission_id where last_modified  >  DATE(?) - interval ' $jobsAutoDeleteAge days' and user_id = ?", [Core::getCurrentDate(), $userId]);
